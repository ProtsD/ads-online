...
public class CommentServiceImpl implements CommentService {
    ...
    public Comments getAllComments(int adId) {
        List<Comment> comments = commentRepository.findAllByAdEntityId(adId)
                .map(entities -> entities.stream().map(commentMapper::toComment).toList())
                .orElseThrow(() -> {
                    String message = String.format("Comments for Ad with id=%d were not found", adId);
                    log.warn(message);
